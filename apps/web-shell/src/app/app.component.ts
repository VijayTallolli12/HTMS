...
export class AppComponent implements OnInit {
  ...
  get currentWorkspaceLabel(): string {
    ...
    if (url.includes('/pms/housekeeping')) return 'Housekeeping Management';
    if (url.includes('/pms/folios')) return 'Cashiering & Folios';
    if (url.includes('/pms/availability')) return 'Inventory & Availability';
    if (url.includes('/pms/engineering')) return 'Engineering & Work Orders';
    if (url.includes('/organization')) return 'Organization Architecture';
    if (url.includes('/health')) return 'System Status';
    if (url.includes('/dashboard')) return 'Executive Dashboard';
