...
| **Front Office (W1-T07)**      | `pms_schema`      | `reservation_assignment_logs`, `reservations` (`assignedRoomId`, `checkInAt`, `checkedInBy`)                         | Room assignment algorithms, arrival processing, guest check-in, operational readiness validation. Source: `docs/06-BUSINESS-WORKFLOWS.md`.                                                                       |
| **Finance / Folios (W1-T08)**  | `finance_schema`  | `folios`, `folio_transactions`, `payments`, `reservations` (`checkOutAt`, `checkedOutBy`, `checkOutIdempotencyKey`)  | Guest folios, charges, payments, departure checkout balance settlement, append-only financial ledger. Source: `docs/W1-T08-CHECKOUT-FOLIO-SETTLEMENT.md`.                                                        |
